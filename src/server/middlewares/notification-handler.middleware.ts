...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'menu.update': { roles: ['owner'], key: 'menu-update', tenantPath: ['restaurant', 'menu'], userPath: [] },
  'table.delete': { roles: ['chef'], key: 'table-deletion', tenantPath: ['restaurant', 'table'], userPath: [] },
  'reservation.update': {
    roles: ['owner'],
    key: 'reservation-update',
    tenantPath: ['restaurant', 'table', 'reservation'],
    userPath: [],
  },
};
...
const tenantRoles: string[] = ['owner', 'chef'];
